...
export default function HoliKitDetail() {
  const { id } = useParams<Params>();
  const { addToCart } = useCart();
  const { isAuthenticated } = useAuth();

  const [currentProduct, setCurrentProduct] = useState<HoliKit | null>(null);
  const [currentVariant, setCurrentVariant] = useState<HoliKitVariant | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        const data: HoliKit = res.data?.product;
        console.log(data);
        if (!data) throw new Error("No product found");

        setCurrentProduct(data);

        // Default variant → first variant OR converted main product
        setCurrentVariant(
          data.variants?.[0] || {
            id: data._id,
            name: data.name,
