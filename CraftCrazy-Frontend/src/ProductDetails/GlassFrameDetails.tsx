// src/Pages/GlassFrameDetails.tsx
...
export default function GlassFrameDetails() {
  const { id } = useParams<Params>();
  const { addToCart } = useCart();
  const { isAuthenticated } = useAuth();

  const [currentProduct, setCurrentProduct] = useState<GlassFrame | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        console.error("GlassFrame fetch error:", err);
