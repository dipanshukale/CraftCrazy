// src/Pages/WomenAccessoryDetailPage.tsx
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        const data: WomenAccessory = res.data?.product;
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
        ...
        <div className="flex-1 flex flex-col gap-4 sm:gap-5">
          <h1 className="text-3xl sm:text-4xl font-serif text-gray-900">{currentVariant?.name || currentProduct.name}</h1>
          ...
          {/* Description */}
          {currentVariant?.description && <p className="text-gray-700 leading-relaxed">{currentVariant.description}</p>}
