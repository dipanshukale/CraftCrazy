...
export type ResinClock = {
  ...
  careInstructions?: string;
  specifications?: Record<string, string>;
  variants?: ResinClockVariant[];
  contents?: string[]; // added
...
export default function ResinClockDetailPage() {
  ...
  const [currentProduct, setCurrentProduct] = useState<ResinClock | null>(null);
  const [currentVariant, setCurrentVariant] = useState<ResinClockVariant | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        const data: ResinClock = res.data?.product;
  ...
  const finalRating = backendRating || 0;
  const finalReviewsCount = backendReviewsCount || 0;
        ...
        <div className="flex-1 flex flex-col gap-4 sm:gap-5">
          ...
          <div className="mt-2 space-y-2 text-gray-700">
            ...
            {currentVariant?.careInstructions && (
              <p>
                <span className="font-semibold">Care Instructions:</span> {currentVariant.careInstructions}
              </p>
            )}
          </div>
